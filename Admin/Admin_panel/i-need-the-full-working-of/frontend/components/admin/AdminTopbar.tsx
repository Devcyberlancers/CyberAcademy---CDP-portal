...
type Notice = { type: string; count: number; message: string };
...
export function AdminTopbar({ title, subtitle, onMenuClick }: AdminTopbarProps) {
  ...
  useEffect(() => {
    void Promise.all([getAdminProfile(), getAdminDashboard()]).then(([admin, dashboard]) => {
      setProfile(admin);
      setNotices(dashboard.notifications.filter((item) => item.count > 0));
    }).catch(() => undefined);
  ...
  return <header className="sticky top-0 z-30 border-b border-portal-line bg-white">
    <div className="relative flex min-h-16 items-center gap-3 px-3 py-3 sm:gap-5 sm:px-5 md:min-h-20 md:px-8">
      ...
      <button type="button" onClick={() => setPanel(panel === "notifications" ? null : "notifications")} className="relative hidden h-10 w-10 shrink-0 place-items-center rounded-md text-slate-600 hover:bg-slate-100 sm:grid" aria-label="Notifications"><Bell size={22} />{notices.length ? <span className="absolute right-1 top-0 grid h-5 min-w-5 place-items-center rounded-full bg-red-500 px-1 text-xs font-bold text-white">{notices.length}</span> : null}</button>
      ...
      {panel ? <div className="absolute right-3 top-[68px] z-50 w-[min(92vw,360px)] rounded-xl border border-portal-line bg-white p-4 shadow-2xl md:right-8 md:top-[78px]">
        ...
        : <div className="space-y-2">{notices.map((item) => <div key={item.type} className="rounded-lg bg-slate-50 p-3 text-sm"><p className="font-bold text-slate-900">{item.message}</p><p className="mt-1 text-xs text-slate-500">{item.count} item{item.count === 1 ? "" : "s"}</p></div>)}{!notices.length ? <p className="py-5 text-center text-sm text-slate-500">No pending notifications.</p> : null}</div>}
      </div> : null}
    </div>
  </header>;
}
