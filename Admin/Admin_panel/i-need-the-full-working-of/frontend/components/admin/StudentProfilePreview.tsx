...
export function StudentProfilePreview({ student, learningRecord, jobs, onClose }: Props) {
  ...
  const profile = student as StudentRecord & { photoDataUrl?: string; photo_data_url?: string; resumeUrl?: string; resumeDataUrl?: string; resumeFileName?: string };
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-6">
            <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_280px]">
              <div className="space-y-5">
                ...
                <PreviewSection icon={<UserRound size={18} />} title="Profile information"><InfoGrid rows={[["Full name", student.name], ["Email", student.email], ["Phone", student.phone], ["Batch", student.batch], ["Department", student.branch], ["Profile status", student.status]]} /></PreviewSection>
              ...
              <aside className="space-y-5"><PreviewSection icon={<FileText size={18} />} title="Documents"><div className="space-y-3">{resume ? <DocumentCard name={profile.resumeFileName || "Resume"} url={resume} /> : <EmptyState text="No resume or uploaded documents are available in the current record." />}</div></PreviewSection><PreviewSection icon={<ExternalLink size={18} />} title="External links">{student.portfolioUrl ? <a href={student.portfolioUrl} target="_blank" rel="noreferrer" className="flex items-center justify-between rounded-xl border border-portal-line p-3 text-sm font-bold text-portal-blue transition hover:bg-blue-50">Portfolio <ExternalLink size={16} /></a> : <EmptyState text="No portfolio link added." />}</PreviewSection></aside>
            </div>
          </div>
        </motion.div>
      </motion.div>
