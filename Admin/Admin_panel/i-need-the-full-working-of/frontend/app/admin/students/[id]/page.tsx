      ...
      {selectedLearningAssessment ? (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/60 p-4" role="dialog" aria-modal="true" aria-label="Assessment details" onMouseDown={(event) => { if (event.currentTarget === event.target) setSelectedLearningAssessment(null); }}>
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
            ...
            <div className="px-5 pb-5"><h3 className="mb-3 font-bold text-slate-950">Student Attempts</h3><div className="space-y-3">{selectedLearningAssessment.attempts.map((attempt) => <div key={attempt.attempt_id} className="rounded-md border border-portal-line p-4"><div className="flex items-center justify-between gap-3"><div><p className="font-bold text-slate-900">Attempt {attempt.attempt_number}</p><p className="mt-1 text-xs capitalize text-slate-500">{attempt.status.replaceAll("_", " ")} · {new Date(attempt.submitted_at ?? attempt.started_at).toLocaleString("en-IN", { timeZone: "Asia/Kolkata", dateStyle: "medium", timeStyle: "short" })} IST</p></div><p className="text-xl font-bold text-portal-blue">{attempt.earned_marks !== undefined && attempt.total_marks ? `${attempt.earned_marks}/${attempt.total_marks} (${attempt.score}%)` : `${attempt.score}%`}</p></div><div className="mt-3 grid gap-2 text-xs text-slate-500 sm:grid-cols-2"><p>Security violations: <b className="text-slate-800">{attempt.violations}</b></p><p>IP address: <b className="text-slate-800">{attempt.ip_address || "Unavailable"}</b></p><p>Browser: <b className="text-slate-800">{attempt.browser || "Unknown"}</b></p><p>Operating system: <b className="text-slate-800">{attempt.operating_system || "Unknown"}</b></p></div><AttemptProctoringTimeline events={attempt.proctoring_events} /></div>)}{!selectedLearningAssessment.attempts.length ? <p className="rounded-md bg-slate-50 p-5 text-sm text-slate-500">This is a real published assessment, but this student has not attempted it yet.</p> : null}</div></div>
          </div>
        </div>
      ) : null}
    </AdminShell>
...
function AttemptProctoringTimeline({ events = [] }: { events?: Array<{ event_type: string; reason?: string; timestamp?: string }> }) {
  if (!events.length) return null;
  return <details className="mt-3 rounded-md bg-slate-50 p-3"><summary className="cursor-pointer text-xs font-bold text-slate-700">Proctoring timeline ({events.length})</summary><div className="mt-2 space-y-2">{events.map((event, index) => <div key={`${event.event_type}-${event.timestamp ?? index}`} className="flex flex-wrap justify-between gap-2 border-t border-slate-200 pt-2 text-xs"><span className="font-bold text-slate-800">{event.event_type.replaceAll("_", " ")}</span><span className="text-slate-500">{event.reason || "Monitoring event"}{event.timestamp ? ` · ${new Date(event.timestamp).toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })} IST` : ""}</span></div>)}</div></details>;
}
