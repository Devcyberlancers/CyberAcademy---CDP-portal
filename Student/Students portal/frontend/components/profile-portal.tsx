        ...
        <div className="grid gap-6 px-6 pb-8 lg:grid-cols-[405px_1fr]">
          <Card className="-mt-16 rounded-[10px] border-0 bg-white p-6 text-center shadow-[0_18px_55px_rgba(17,24,74,.12)]">
            <label className="relative mx-auto flex h-32 w-32 cursor-pointer items-center justify-center overflow-hidden rounded-full bg-[#f0f6ff]">
              <input ref={photoInputRef} type="file" accept="image/*" className="sr-only" onChange={(event) => updateProfilePhoto(event.target.files?.[0])} />
              {draft.photoDataUrl ? (
...
function PhotoCropEditor({ source, crop, onCropChange, onChoosePhoto, onRemovePhoto, onApply, onCancel }: { source: string; crop: { x: number; y: number; size: number }; onCropChange: (crop: { x: number; y: number; size: number }) => void; onChoosePhoto: () => void; onRemovePhoto: () => void; onApply: () => void; onCancel: () => void }) {
