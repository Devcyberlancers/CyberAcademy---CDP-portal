...
import { defaultProctoringConfig, type ProctoringConfig } from "@/lib/proctoring/types";
...
export function ExamReadinessDialog({title,onClose,onProceed,config}:Props){
 ...
 const videoRef=useRef<HTMLVideoElement>(null),effectiveConfig={...defaultProctoringConfig,...config},configRef=useRef(effectiveConfig),env=environment();
 configRef.current=effectiveConfig;
 const cameraRequired=effectiveConfig.camera;
 const systemChecks=[
 ...
 useEffect(()=>{if(stage!==3||!cameraRequired||!mediaReady||!videoRef.current)return;const stream=(window as StreamWindow).__cyberAcademyMediaStream;if(stream){videoRef.current.srcObject=stream;void videoRef.current.play()}},[cameraRequired,stage,mediaReady]);
 ...
 async function allowMedia(){
  ...
  try{
   const stream=await navigator.mediaDevices.getUserMedia({video:cameraRequired?{width:{ideal:640},height:{ideal:360},facingMode:"user"}:false,audio:effectiveConfig.microphone?{echoCancellation:true,noiseSuppression:true}:false});
  ...
  }catch{setMediaReady(false);setError(cameraRequired?"Camera and microphone permission is required. Allow access and try again.":"Microphone permission is required. Allow access and try again.")}
 ...
 async function start(){
  ...
  }catch{setCountdown(null);setError(cameraRequired?"Fullscreen permission is required. Keep camera, microphone, and screen sharing active.":"Fullscreen permission is required. Keep microphone and screen sharing active.")}
 }
 if(countdown!==null)return <section className="fixed inset-0 z-[130] grid place-items-center bg-[#07142f] text-white"><div className="text-center"><p className="text-sm font-bold uppercase tracking-[.24em] text-blue-200">Secure test starting</p><div className="mx-auto mt-6 grid h-36 w-36 place-items-center rounded-full border-4 border-white/20 text-7xl font-bold shadow-[0_0_0_10px_rgba(49,85,255,.25)]">{countdown||"GO"}</div><p className="mt-6 text-lg">{cameraRequired?"Camera, microphone, screen sharing, and tab monitoring are active.":"Microphone, screen sharing, and tab monitoring are active. Camera monitoring is disabled."}</p></div></section>;
 return <section className="fixed inset-0 z-[100] grid place-items-center bg-slate-950/60 p-4" role="dialog" aria-modal="true">
  <div className="flex max-h-[88vh] w-full max-w-4xl flex-col overflow-hidden rounded-[18px] bg-white shadow-2xl">
   <header className="mx-8 flex items-center justify-between border-b py-6"><div><h2 className="text-[22px] font-bold">{stage===0?"Terms And Instructions":stage===1?"Checking Test Validity And Availability":stage===2?"Browser And System Check":stage===3?(cameraRequired?"Checking Webcam, Mic And Authentication":"Checking Mic And Authentication"):stage===4?"Screen Access Check":"Ready To Begin"}</h2><p className="mt-1 text-sm text-slate-500">{title}</p></div><button onClick={close} aria-label="Close"><X size={27} className="text-slate-500"/></button></header>
   <div className="min-h-[440px] overflow-y-auto px-8 py-7">
    {stage===0?<><h3 className="text-xl font-bold text-slate-600">Please carefully read and agree to the below:</h3><ul className="mt-7 space-y-4">{terms.map((item)=>!cameraRequired&&item.startsWith("Enable cookies")?"Enable cookies and allow microphone, screen sharing, and fullscreen permissions. Camera permission is not required for this test.":item).map((item)=><li key={item} className="flex gap-4 text-[15px] leading-6 text-slate-600"><span className="mt-2 h-3 w-3 shrink-0 rotate-45 bg-[#969696]"/>{item}</li>)}</ul></>:null}
    ...
    {stage===3?<div className="mx-auto max-w-3xl">{cameraRequired?<div className="grid min-h-64 place-items-center overflow-hidden rounded-xl bg-slate-950">{mediaReady?<video ref={videoRef} muted playsInline className="max-h-72 w-full object-contain"/>:<Video className="h-24 w-24 text-white/60" strokeWidth={1}/>}</div>:<div className="grid min-h-48 place-items-center rounded-xl border border-emerald-200 bg-emerald-50 p-8 text-center"><ShieldCheck className="h-20 w-20 text-emerald-600" strokeWidth={1.25}/><div><b className="text-lg text-slate-900">Camera not required for this test</b><p className="mt-2 text-sm text-slate-600">The administrator disabled camera, face, person, and phone monitoring for this test.</p></div></div>}<div className="mt-5 flex items-center gap-3 rounded-lg bg-[#f6f7fb] p-4"><Mic className={mediaReady?"text-emerald-600":"text-slate-500"}/><span>{mediaReady?(cameraRequired?"Camera and microphone are live and will remain on until submission.":"Microphone is live and will remain on until submission."):(cameraRequired?"Allow camera and microphone to continue.":"Allow microphone to continue.")}</span></div></div>:null}
    ...
    {stage===5?<div className="mx-auto max-w-xl py-16 text-center"><ShieldCheck className={`mx-auto h-28 w-28 ${monitoringReady?"text-emerald-600":"text-[#3155ff]"}`}/><h3 className="mt-5 text-2xl font-bold">{checking?"Preparing assessment monitoring...":monitoringReady?"Proctoring ready.":"Assessment monitoring could not be initialized."}</h3><p className="mt-3 text-slate-600">{cameraRequired?"Camera, face, person and phone detection, browser monitoring, audio, and screen sharing were verified.":"Browser monitoring, audio, and screen sharing were verified. Camera-based monitoring is disabled by the administrator."}</p></div>:null}
