...
function CourseQuiz({ module, answers, onChoose, error, result, onSubmit, onClose }: { module?: CourseModule; answers: Record<string, string>; onChoose: (index: number, value: string) => void; error: string; result: { score: number; passed: boolean } | null; onSubmit: () => void; onClose: () => void }) {
  const [current, setCurrent] = useState(0); const [bookmarked, setBookmarked] = useState<Set<number>>(() => new Set()); const [visited, setVisited] = useState<Set<number>>(() => new Set([0])); const questions = module?.generatedQuestions ?? []; const question = questions[current];
  useEffect(() => { const block=(event:Event)=>event.preventDefault(); const keys=(event:KeyboardEvent)=>{if((event.ctrlKey||event.metaKey)&&["c","v","x","a"].includes(event.key.toLowerCase()))event.preventDefault();}; const hidden=()=>{if(document.hidden&&!result)onSubmit();}; document.addEventListener("copy",block);document.addEventListener("cut",block);document.addEventListener("paste",block);document.addEventListener("contextmenu",block);document.addEventListener("keydown",keys);document.addEventListener("visibilitychange",hidden);return()=>{document.removeEventListener("copy",block);document.removeEventListener("cut",block);document.removeEventListener("paste",block);document.removeEventListener("contextmenu",block);document.removeEventListener("keydown",keys);document.removeEventListener("visibilitychange",hidden);};},[onSubmit,result]);
  function go(index:number){const safe=Math.max(0,Math.min(questions.length-1,index));setCurrent(safe);setVisited((value)=>new Set(value).add(safe));} function toggle(){setBookmarked((value)=>{const next=new Set(value);next.has(current)?next.delete(current):next.add(current);return next;});}
