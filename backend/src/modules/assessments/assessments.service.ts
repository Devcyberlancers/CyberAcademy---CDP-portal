...
import {
  assignment_attempts_status,
  assignment_security_settings_violation_policy,
  Prisma,
} from "@prisma/client";
...
export class AssessmentsService {
  ...
  private async collection(kind: string, course?: string, batch?: string) {
    ...
    const row = await this.prisma.assessment_collections.findUnique({
      where: { storage_key },
    });
  ...
  private nativeId(course: string, assessment: string) {
    ...
    return raw.length <= 80
      ? raw
      : `course:${course.slice(0, 40)}:${createHash("sha256").update(raw).digest("hex").slice(0, 24)}`;
  }
      ...
      const correct =
        options.find(
          (option: { id: string; text: string }) =>
            option.id === answer ||
            option.text.trim().toLowerCase() === answer.trim().toLowerCase(),
        )?.id ?? options[0].id;
      return {
        id,
        text: String(
          question.text || question.title || `Question ${index + 1}`,
        ),
        options,
  ...
  private settingData(id: string, item: any, published: boolean) {
    ...
    return {
      ...
      active: published,
      enabled: true,
      camera_enabled: item.cameraRequired ?? item.cameraEnabled ?? true,
      questions_json: this.questions(item.questions || []),
      ...
      Prisma.assignment_security_settingsUncheckedCreateInput,
      "assignment_id"
    >;
  }
  ...
  async saveCollection(
    ...
    const published =
      kind === "standalone" ||
      Boolean(courseRow && ["active", "published"].includes(courseRow.status));
    const namespace = `${course ?? "standalone"}:${selectedBatch}`;
    const ids = assessments.map((item) =>
      this.nativeId(namespace, String(item.id || "assessment")),
    );
    await this.prisma.$transaction(async (tx) => {
      await tx.assessment_collections.upsert({
        ...
        create: {
          storage_key,
          kind,
          course_key: course ?? null,
          payload: JSON.stringify(assessments),
          updated_at: new Date(),
        },
        update: {
          payload: JSON.stringify(assessments),
      ...
      await tx.assignment_security_settings.deleteMany({
        where: {
          assignment_id: { startsWith: `course:${namespace}:`, notIn: ids },
        },
      });
      if (courseRow)
        await tx.courses.update({
          where: { id: courseRow.id },
          data: { assessments: assessments.length, updated_at: new Date() },
        });
    });
    return { saved: true };
  }
  ...
  private security(item: any) {
    return {
      ...
      cameraEnabled: item.camera_enabled,
      requireFullscreen: item.require_fullscreen,
      endOnFullscreenExit: item.end_on_fullscreen_exit,
      endOnTabSwitch: item.end_on_tab_switch,
      endOnBlur: item.end_on_blur,
      disableRightClick: item.disable_right_click,
      disableCopy: item.disable_copy,
      disablePaste: item.disable_paste,
      disableCut: item.disable_cut,
      disableDrag: item.disable_drag,
      disableTextSelection: item.disable_text_selection,
      disablePrinting: item.disable_printing,
      disableSavePage: item.disable_save_page,
      disableInspectShortcuts: item.disable_inspect_shortcuts,
      randomizeQuestionOrder: item.randomize_question_order,
      randomizeOptionOrder: item.randomize_option_order,
      autoSaveAnswers: item.auto_save_answers,
      autoSubmitOnTimerEnd: item.auto_submit_on_timer_end,
      logDeviceInfo: item.log_device_info,
      logBrowserInfo: item.log_browser_info,
      logIpAddress: item.log_ip_address,
      logSessionChanges: item.log_session_changes,
  ...
  async assignments(email?: string, requestedAssignmentId?: string) {
    const profile = email
      ? await this.prisma.student_profiles.findUnique({
          where: { email: email.toLowerCase() },
          select: { batch: true },
        })
      : null;
    const batch = profile?.batch ?? "2026 A";
    const collection = await this.prisma.assessment_collections.findUnique({
      where: {
        storage_key: this.collectionKey("standalone", undefined, batch),
      },
    });
    ...
    const currentIds = Array.isArray(current)
      ? current.flatMap((item: any) => {
          ...
          return batch === "2026 A"
            ? [scoped, this.nativeId("standalone", id)]
            : [scoped];
        })
      : [];
    const assignmentIds = requestedAssignmentId
    ...
    const allAttempts =
      email && rows.length
        ? await this.prisma.assignment_attempts.findMany({
            where: {
              assignment_id: { in: rows.map((item) => item.assignment_id) },
    ...
    return rows.map((item) => {
      ...
      return {
        ...
        canStart:
          remaining > 0 ||
          Boolean(latest?.status === "in_progress" && item.resume_allowed),
        questionCount: Array.isArray(item.questions_json)
          ? item.questions_json.length
          : 0,
        attempts: attempts.map((attempt) =>
          this.summary(attempt, item.assignment_title),
        ),
  ...
  async start(
    ...
    const assignment =
      await this.prisma.assignment_security_settings.findUnique({
    ...
    if (existing?.status === "in_progress") {
      if (assignment.resume_allowed)
        return this.attemptResponse(assignment, existing, "resume");
      ...
      throw new ConflictException({
        ...
        message:
          existing?.status === "completed"
            ? "Assignment already completed. No attempts remaining."
            : "You have used all available attempts.",
      });
    }
    const questions: any[] = Array.isArray(assignment.questions_json)
      ? (assignment.questions_json as any[])
      : [];
    const questionOrder = this.shuffled(
      questions.map((q) => q.id),
      assignment.randomize_question_order,
    );
    const optionOrder = Object.fromEntries(
      questions.map((q) => [
        ...
        this.shuffled(
          ...
          assignment.randomize_option_order,
        ),
      ]),
    );
    ...
    const attempt = await this.prisma.assignment_attempts.create({
      data: {
        ...
        status: assignment_attempts_status.in_progress,
        started_at: now,
        ended_at: null,
        ...
        operating_system: dto.device.operating_system,
        screen_resolution: dto.device.screen_resolution,
        user_agent: dto.device.user_agent || userAgent,
        ip_address: ip,
        question_order_json: questionOrder,
        option_order_json: optionOrder,
        answers_json: {},
        score: 0,
      },
    });
    await this.event(attempt.id, {
      event_type: "ATTEMPT_STARTED",
      reason: "",
      details: { assignmentId, attemptNumber: attempts.length + 1 },
    });
  ...
  private publicQuestions(assignment: any, attempt: any) {
    const questions: any[] = Array.isArray(assignment.questions_json)
      ? assignment.questions_json
      : [];
    const byId = new Map(questions.map((q) => [q.id, q]));
    const order: any[] = Array.isArray(attempt.question_order_json)
      ? attempt.question_order_json
      : [];
    const optionOrder: any = attempt.option_order_json ?? {};
    return order.map((id) => {
      const question = byId.get(id);
      const options = new Map(
        (question?.options ?? []).map((o: any) => [o.id, o]),
      );
      return {
        id,
        text: question?.text,
        options: (optionOrder[id] ?? [])
          .map((oid: string) => options.get(oid))
          .filter(Boolean)
  ...
  private async attemptResponse(
    ...
    const events = await this.prisma.assignment_events.findMany({
      where: { attempt_id: attempt.id },
      orderBy: { created_at: "desc" },
    });
    const endsAt = new Date(
      attempt.started_at.getTime() + assignment.duration_minutes * 60_000,
    );
    return {
      action,
      ...
      resumeAllowed: assignment.resume_allowed,
      ...
      questions: this.publicQuestions(assignment, attempt),
  ...
  async saveAnswer(id: number, dto: SaveAnswerDto) {
    const attempt = await this.prisma.assignment_attempts.findUnique({
      where: { id },
    });
    ...
    const answers: any = {
      ...((attempt.answers_json as any) ?? {}),
      [dto.question_id]: dto.option_id,
    };
    await this.prisma.$transaction([
      this.prisma.assignment_attempts.update({
        where: { id },
        data: { answers_json: answers },
      }),
      this.prisma.assignment_events.create({
        data: {
          attempt_id: id,
          event_type: "ANSWER_SAVED",
          reason: dto.question_id,
          details_json: { clientTimestamp: dto.client_timestamp ?? null },
          created_at: new Date(),
        },
      }),
    ]);
  ...
  async event(id: number, dto: EventDto) {
    if (!(await this.prisma.assignment_attempts.findUnique({ where: { id } })))
      throw new NotFoundException("Attempt not found");
    await this.prisma.assignment_events.create({
      data: {
        attempt_id: id,
        event_type: dto.event_type,
        reason: dto.reason,
        details_json: dto.details,
        created_at: new Date(),
      },
    });
    return { ok: true };
  ...
  async events(
    ...
    if (!Array.isArray(events) || events.length > 100)
      throw new UnprocessableEntityException(
        "A proctoring event batch may contain at most 100 events",
      );
    const rows = events.map((event) => {
      ...
      if (!eventType)
        throw new UnprocessableEntityException(
          "Every proctoring event requires an event_type",
        );
    ...
    if (rows.length) {
      const violationCount = rows.filter((row) => {
        const severity = String(
          (row.details_json as Record<string, unknown>)?.severity || "",
        );
        return severity === "warning" || severity === "critical";
      }).length;
      await this.prisma.$transaction([
        this.prisma.assignment_events.createMany({ data: rows }),
        ...(violationCount
          ? [
              this.prisma.assignment_attempts.update({
                where: { id },
                data: { violations: { increment: violationCount } },
              }),
            ]
          : []),
      ]);
    ...
    const earnedMarks = questions.reduce(
      (sum, question) =>
        ...
        (answers[question.id] === question.correct_option_id
          ? Math.max(1, Number(question.marks) || 1)
          : 0),
      0,
    );
  ...
  async close(id: number, dto: CloseAttemptDto, terminate = false) {
    ...
    const assignment =
      await this.prisma.assignment_security_settings.findUnique({
        where: { assignment_id: attempt.assignment_id },
      });
    if (!assignment) throw new NotFoundException("Assessment not found");
    if (attempt.status !== "in_progress")
      return this.attemptResponse(assignment, attempt);
    const answers = {
      ...((attempt.answers_json as any) ?? {}),
      ...dto.answers,
    };
    const status = terminate
      ? assignment_attempts_status.terminated
      : dto.auto_submitted
        ? assignment_attempts_status.auto_submitted
        : assignment_attempts_status.completed;
    const updated = await this.prisma.assignment_attempts.update({
      ...
      data: {
        ...
        status,
        ended_at: new Date(),
        auto_submitted: dto.auto_submitted,
        ...
        score: this.score(
          Array.isArray(assignment.questions_json)
            ? (assignment.questions_json as any[])
            : [],
          answers,
        ),
    ...
    await this.event(id, {
      event_type: terminate ? "ATTEMPT_TERMINATED" : "ATTEMPT_SUBMITTED",
      reason: dto.reason,
      details: { autoSubmitted: dto.auto_submitted },
    });
  ...
  private summary(attempt: any, title = "") {
    ...
    return {
      ...
      durationSeconds: Math.floor(
        (ended.getTime() - attempt.started_at.getTime()) / 1000,
      ),
      terminationReason: attempt.termination_reason,
      autoSubmitted: attempt.auto_submitted,
      violations: attempt.violations,
      score: attempt.score,
      answeredCount: Object.keys((attempt.answers_json as any) ?? {}).length,
      browser: attempt.browser,
      operatingSystem: attempt.operating_system,
      screenResolution: attempt.screen_resolution,
      userAgent: attempt.user_agent,
      ipAddress: attempt.ip_address,
      riskLevel:
        attempt.status === "terminated" || attempt.violations > 1
          ? "red"
          : attempt.violations
            ? "yellow"
            : "green",
  ...
  async attempts() {
    const rows = await this.prisma.assignment_attempts.findMany({
      orderBy: { started_at: "desc" },
    });
    return rows.map((row) => {
      ...
      return {
        ...
        device: summary.operatingSystem,
  ...
  async adminAttempts(filters: {
    ...
    if (filters.scope === "standalone") {
      const collection = await this.prisma.assessment_collections.findUnique({
        where: {
          storage_key: this.collectionKey(
            "standalone",
            undefined,
            filters.batch,
          ),
    ...
    const batchEmails = filters.batch
      ? (
          await this.prisma.student_profiles.findMany({
            where: { batch: filters.batch.trim() },
            select: { email: true },
          })
    ...
    const where: Prisma.assignment_attemptsWhereInput = {
      ...
      ...(scopedAssignmentIds
        ...
        : {}),
      ...(filters.status
        ? { status: filters.status as assignment_attempts_status }
        : {}),
      ...(filters.email
        ? { student_email: { contains: filters.email } }
        : batchEmails
          ? { student_email: { in: batchEmails } }
          : {}),
    };
  ...
  async attemptDetail(id: number, answersOnly = false) {
    ...
    return {
      ...
      questions: Array.isArray(assignment.questions_json)
        ? assignment.questions_json.length
        : 0,
      answers,
      events: events.map((event) => ({
        eventId: event.id,
  ...
  async nativeAssessments() {
    ...
    return rows.map((item) => ({
      ...
      resume_allowed: item.resume_allowed,
      max_attempts: item.max_attempts,
      question_count: Array.isArray(item.questions_json)
        ? item.questions_json.length
        : 0,
  ...
  async upsertNative(dto: NativeAssessmentDto) {
    if (
      !["warning", "auto_submit", "end_exam"].includes(dto.violation_policy)
    ) {
      throw new UnprocessableEntityException("Invalid violation_policy");
    }
    const base = this.settingData(
      dto.assignment_id,
      {
        title: dto.assignment_title,
    ...
    const row = await this.prisma.assignment_security_settings.upsert({
      ...
      create: {
        ...
        available_until: dto.available_until
          ...
          : null,
        resume_allowed: dto.resume_allowed,
        enabled: dto.enabled,
        camera_enabled: dto.camera_enabled,
        require_fullscreen: dto.require_fullscreen,
        end_on_fullscreen_exit: dto.end_on_fullscreen_exit,
        end_on_tab_switch: dto.end_on_tab_switch,
        end_on_blur: dto.end_on_blur,
        randomize_question_order: dto.randomize_question_order,
        randomize_option_order: dto.randomize_option_order,
        auto_save_answers: dto.auto_save_answers,
        auto_submit_on_timer_end: dto.auto_submit_on_timer_end,
        violation_policy:
          dto.violation_policy as assignment_security_settings_violation_policy,
      },
      update: {
        ...base,
